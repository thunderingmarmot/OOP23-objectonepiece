...
\subsection*{Prototipo}
Il mondo di gioco contiene le sezioni ed è responsabile della creazione di nuove sezioni e di caricare quelle salvate.
%
Ogni sezione contiene il giocatore e tutte o solo alcune delle seguenti entità: nemici, barili, mine e isole.
%
Il giocatore si deve poter muovere nella sezione corrente e deve poter cambiare sezione andando nei suoi bordi. Deve inoltre poter sparare alle navi nemiche e collidere con tutte le entità.
%
Sia i nemici che il giocatore sono entrambi navi che possono effettuare collisioni con le altre navi e entità.
%
Una nave è composta da 4 componenti che sono: la chiglia, la prua, la vela e l'arma, inoltre la distruzione di ogni componente comporta una conseguenza differente alle azioni che può svolgere la nave.
%
Per quanto riguardo isole, barili e mine, possono solo essere collisi dalle navi.
%
Se il giocatore collide con le mine esse gli procurano danno alla nave, mentre se collide con i barili riceverà dell'esperienza.
%
Quando il giocatore interagisce con le isole deve poter riparare la nave e salvare l'esperienza e la sezione corrente.
%
I nemici devono poter vedere il giocatore se entra nel loro raggio visivo e devono seguirlo ed in caso sparargli se rientra nel raggio dell'arma. Essi non seguono il giocatore se cambia sezione.
...
%
Avviando il gioco, l'utente si troverà sullo schermo una finestra con in centro una griglia blu (mappa di gioco) e un HUD (denotato dal testo "Player info") sulla parte destra.
%
...
%
Quando il giocatore cerca di muoversi sulle caselle del bordo, si sposta su una sezione diversa del mondo e non potrà più tornare alla precedente ammenoché non l'abbia salvata.
%
...
%
Il range di sparo è di 3 caselle sia per il player che per i nemici e fa un danno AoE (Area of Effect) di 3x3 caselle.
%
...
\end{itemize}
Inoltre, nel HUD sono presenti anche due pulsanti.
%
Il primo con l'icona di una tavola di legno consente al giocatore di riparare la nave (curarsi) utilizzando i barili raccolti nella mappa.
%
